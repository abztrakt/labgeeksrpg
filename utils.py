    ...
    def formatday(self, day, weekday):
        if day != 0:
            ...
            if day in self.shifts:
                ...
                body = '<p><a href="/chronos/%s/%s/%s/%s"><i class="icon-list-alt icon-large"></i><span class="long">Total Hours: <strong class="hours">' % (self.user.username, self.year, self.month, day) + str(total_hours) + '</strong></span></a></p>'
                s += '%s' % (body)
    ...
    def formatday(self, day, weekday):
        if day != 0:
            ...
            if date.today() == date(self.year, self.month, day):
                cssclass += ' today'
            if day in self.shifts:
                s += '<p><a href="/chronos/report/%s/%s/%s"><i class="icon-list-alt icon-large"></i><span class="long">Shift Details</span></a></p>' % (self.year, self.month, day)
